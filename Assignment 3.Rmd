...
The data already looks more normal, but there is still a heavy positive skew. Let's transfor the data:
...
```{r normalising data to account for individual reading times}
g_by_ID <- group_by(dfM, ID)
...
t.test(rt ~ word, data = HJdf, var.equal=TRUE)
...
ggplot(HJdf,
  ...
  geom_errorbar(
    aes(width = 0.2,
      ymin = mean(HJdf$rt) - sd(HJdf$rt),
      ymax = mean(HJdf$rt) + sd(HJdf$rt)
    ) ) +
          theme_minimal()
